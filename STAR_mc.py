...
from utils import evaluation_batch, WarmCosineScheduler, setup_seed, get_logger, HybridMiningLoss
...
def main(args):
    ...
    if 'dinov2' in args.encoder:
        ...
        if 'l' in args.encoder:
            ...
            repo_dir = './dinov3'
            Dinov3_model_path = '/data/cj/INP-Former/dinov3_vits16_pretrain_lvd1689m-08c60483.pth'
    ...
    if args.phase == 'train':
        ...
        # define optimizer
        optimizer = StableAdamW([{'params': trainable.parameters(), 'lr':1e-3}],
        ...
        for epoch in range(args.total_epochs):
            ...
            for img, _ in tqdm(train_dataloader, desc='training'):
                ...
                img = img.to(device)
                en, de, sp_loss = model(img)
                sf_loss = criterion(de, en) 
                loss = sf_loss + 0.1 * sp_loss
            ...
            if (epoch + 1) % args.save_interval == 0 :
                ...
                for item, test_data in zip(args.item_list, test_data_list):
                    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, shuffle=False,
                                                                  num_workers=8)
                    results = evaluation_batch(model,  test_dataloader, device, args.vis_enable,max_ratio=0.01, resize_mask=256)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/data/cj/mvtec') # Replace it with your path.
    # save info
    parser.add_argument('--save_dir', type=str, default='./saved_results')
    parser.add_argument('--save_name', type=str, default='STAR_Multi-class')
    ...
    parser.add_argument('--phase', type=str, default='train')  #'test' or 'train'
    parser.add_argument('--save_interval',type=int, default=10) 

    args = parser.parse_args()
    args.save_name = args.save_name + f'_dataset={args.dataset}_Encoder={args.encoder}_Resize={args.input_size}_Crop={args.crop_size}_INP_num={args.INP_num}_Batch_size={args.batch_size}'
